...
async function setTextAndProcess(wrapper: VueWrapper) {
  const textarea = wrapper.get('[data-testid="input-textarea"] textarea');
...
describe('SidepanelApp', () => {
  ...
  it('should render initial state correctly', () => {
    expect(wrapper.get('[data-testid="input-textarea"] textarea').text()).toBe('');
  ...
  describe('Context Menu Behavior', () => {
    ...
    it('should automatically process text when selected from context menu', async () => {
      ...
      await flushPromises();
      expect((wrapper.get('[data-testid="output-textarea"] textarea').element as HTMLTextAreaElement).value).toContain(PROCESSED_TEXT);
    });
    ...
    it('should not process when source and target languages are the same and summarize is false', async () => {
      ...
      await wrapper.getComponent(VSelect).setValue(sameLanguage);
      await flushPromises();
      // Ensure summarize is unchecked
      const checkbox = wrapper.get('[data-testid="summarize-checkbox"] input');
  ...
  describe('Process Button Behavior', () => {
    ...
    it('should enable button when source language is detected to be different from target language', async () => {
      ...
      await wrapper.getComponent(VSelect).setValue(targetLanguage);
      await flushPromises();
    ...
    it('should enable button after switching target language during processing', async () => {
      mockAIService.processText.mockReturnValue(new Promise(() => { })); // Never resolves
      await wrapper.getComponent(VSelect).setValue(mockSupportedLanguages[1]);
      await setTextAndProcess(wrapper);
      const button = wrapper.get('[data-testid="process-button"]');
      expect(button.attributes('disabled')).toBeDefined();

      // Change target language during processing
      await wrapper.getComponent(VSelect).setValue(mockSupportedLanguages[2]);
      await flushPromises();
  ...
  describe('Model Downloading State', () => {
    ...
    it('should hide model download message when target language changes', async () => {
      ...
      await wrapper.getComponent(VSelect).setValue(mockSupportedLanguages[0]);
      await flushPromises();
    ...
    it('should hide model download message when cancel button is clicked', async () => {
      ...
      const cancelButton = wrapper.get('[data-testid="model-download-card"] button');
  ...
  describe('API Availability Warning', () => {
    it('should show warning when native browser APIs are not available', async () => {
      ...
      expect(wrapper.get('[data-testid="api-warning-container"]').text()).toContain('apiWarning');
    });
  ...
  describe('Default Language Loading', () => {
    ...
    it('should use default language when browser language is not supported', async () => {
      ...
      const targetSelect = wrapper.findComponent(VSelect);
      expect(targetSelect.text()).toContain(fallbackLanguage);
    });

    it('should populate language selector with available languages', () => {
      const targetSelect = wrapper.findComponent(VSelect);
      const listItems = targetSelect.findAll('option');
      expect(listItems).toHaveLength(mockSupportedLanguages.length);
    });
  });
