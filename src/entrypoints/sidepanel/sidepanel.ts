...
export class SidepanelApp {
  #processTextService = getProcessTextService();
  ...
  async #processText(): Promise<void> {
    ...
    try {
      const response = await this.#processTextService.processText(this.#state.text, {
      ...
      // TODO: revisar si modelStatus no debe permanecer
  ...
  #render(): void {
    ...
    if (container.innerHTML === '') {
      ...
      if (processRow) {
        const app = createApp(ProcessControls);
